...
define(function(require)
{
	...
	var MobileJoystick = require('UI/Components/MobileUI/MobileJoystick');
	//const MobileJoystick = require("UI/Components/MobileUI/MobileJoystick");
...
// ปุ่มคลิกขวา (แตะแล้วคลิกขวาทันทีที่ตำแหน่งเคอร์เซอร์เมาส์)
document.getElementById('rightClickButton')?.addEventListener('touchstart', function(e) {
  e.preventDefault();

  // ใช้ตำแหน่งเคอร์เซอร์เมาส์แทน touch
  const x = window._mouseX || 0;
  const y = window._mouseY || 0;
...
// ติดตามตำแหน่งเคอร์เซอร์เมาส์ล่าสุดไว้ใน window._mouseX / _mouseY
...
this.ui.find('#f3Button').on('touchstart mousedown', function(e) {
	keyPress(114); // F3
	stopPropagation(e);
});
this.ui.find('#f4Button').on('touchstart mousedown', function(e) {
	keyPress(115); // F4
	stopPropagation(e);
});
