...
PACKET.ZC.ACK_TRADE_BUYING_STORE = function PACKET_ZC_ACK_TRADE_BUYING_STORE(fp) {
	const index = fp.readShort();
	const count = fp.readShort();
	const zeny  = fp.readLong();

	// ลดจำนวนในร้าน VendingShop (ฝั่งผู้ชมร้าน)
	const item = VendingShop.getItemByIndex(index);
	if (item) {
		item.count -= count;

		if (item.count > 0) {
			VendingShop.ui.find('.item[data-index="' + pkt.index + '"] .amount_').text(item.count + ' ea');

		
		} else {
			VendingShop.list.splice(VendingShop.list.indexOf(item), 1);
			VendingShop.ui.find('.item[data-index="' + index + '"]').remove();
		}

		if (VendingShop.list.length === 0) {
			VendingShop.onSubmit();
		}
	}
};
