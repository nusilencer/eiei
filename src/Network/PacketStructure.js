...
PACKET.ZC.ACK_TOUSESKILL = function PACKET_ZC_ACK_TOUSESKILL(fp, end) {
	...
	if ((CLASSIC && PACKETVER.value >= 20181121) || (RENEWAL && PACKETVER.value >= 20180704)) {
		this.NUM = fp.readLong();        // ? µéÍ§ãÊè à¾×èÍãËé¢éÍ¤ÇÒÁáÊ´§
...
const entity = EntityManager.get(Session.EntityAID);
	// ? ¡ÓË¹´ flag ÇèÒ skill ¹ÕéÅéÁàËÅÇ
	Session.skipNextSkillId = this.SKID;
	Session.skipNextSkillAnimation = true;
	...
	PACKET.ZC.AUTORUN_SKILL = function PACKET_ZC_AUTORUN_SKILL(fp, end) {
		...
		this.data.upgradable = fp.readUChar();
		const EntityManager = require('Renderer/EntityManager');
    const entity = EntityManager.get(this.AID);
    if (entity) {
        entity.lastSkillId = this.SKID;
        console.log('[NOTIFY_SKILL2] ? skillId:', this.SKID, ' entity:', this.AID);
    }
	};
...
// 0x9e6
PACKET.ZC.ACK_TRADE_BUYING_STORE = function PACKET_ZC_ACK_TRADE_BUYING_STORE(fp) {
	this.index = fp.readShort(); // index à¸‚à¸­à¸‡à¹„à¸­à¹€à¸—à¹‡à¸¡à¹ƒà¸™à¸£à¹‰à¸²à¸™
	this.count = fp.readShort(); // à¸ˆà¸³à¸™à¸§à¸™à¸—à¸µà¹ˆà¸‹à¸·à¹‰à¸­à¸ªà¸³à¹€à¸£à¹‡à¸ˆ
	this.zeny  = fp.readLong();  // à¸ˆà¸³à¸™à¸§à¸™à¹€à¸‡à¸´à¸™à¸—à¸µà¹ˆà¸ˆà¹ˆà¸²à¸¢
};
PACKET.ZC.ACK_TRADE_BUYING_STORE.size = 8;
	...
	if (entity && entity.setAction) {
		// µÑÇÍÂèÒ§: ¶éÒ status index ¹Õé¤×Í Freeze  Â×¹à©Â
		if (this.index === 1 /* ÊÁÁØµÔÇèÒ 1 = Frozen */) {
			entity.setAction({
			...
			console.log('[STATE_CHANGE4] Entity', this.AID, 'µÔ´Ê¶Ò¹Ð index', this.index, ' ºÑ§¤Ñº IDLE');
		}
	}
};
...
PACKET.ZC.USE_SKILL2 = function PACKET_ZC_USE_SKILL2(fp, end) {
	...
	// ? à¾ÔèÁÊèÇ¹¹Õéà¾×èÍºÑ¹·Ö¡ skillId ãËé¡Ñº entity
	const EntityManager = require('Renderer/EntityManager');
	const entity = EntityManager.get(this.srcAID);
	if (entity) {
		entity.lastSkillId = this.SKID;
		console.log('[NOTIFY_SKILL2] ?? skillId:', this.SKID, '?? entity:', this.srcAID);
	}
};
...
PACKET.ZC.BROADCAST_ITEMREFINING_RESULT = function PACKET_ZC_BROADCAST_ITEMREFINING_RESULT(fp, end) {
    this.charName = fp.readString(NAME_LENGTH);
    this.itemId = fp.readUShort(); // < 20181121
    this.refineLevel = fp.readChar();
    this.status = fp.readChar();
	};
	PACKET.ZC.BROADCAST_ITEMREFINING_RESULT.size = 32;
